  ...
  const name  = localStorage.getItem("name") || localStorage.getItem("userName") || "Plumber";

  if (!phone || role !== "plumber") {
    window.location.replace("login.html");
    throw new Error("Plumber role required");
  }
  ...
  function renderRoleSidebar() {
    ...
    if (nav) {
      nav.innerHTML = `
        <a class="active" href="plumber_requests.html"><i class="fa-solid fa-table-cells-large"></i>Dashboard</a>
    ...
    if (info) {
      info.innerHTML = `
        <div><i class="fa-solid fa-bolt"></i>Plumber workspace</div>
  ...
  function setWorkerAvatar(imageValue, displayName) {
    ...
    document.querySelectorAll("[data-ns-avatar]").forEach(el => {
      el.innerHTML = imageSrc
        ? `<img src="${imageSrc}" alt="${displayName || "Plumber"}">`
        : initial;
    });
  ...
  async function loadSidebarProfileAvatar() {
    try {
      ...
      if (profileResult.status === "fulfilled" && profileResult.value.ok) {
        ...
        console.log("Plumber profile user:", user);
      }
      ...
      const profileImage = user.avatarBase64 ||
        ...
        "";
      const displayName = user.name || name || "Plumber";
      ...
      console.error("Could not load plumber sidebar avatar", e);
  ...
  async function fetchRequests(){
    ...
    try {
      const res = await fetch(`${API_BASE}/bookings/chosen/plumber/${encodeURIComponent(phone)}`);
      const bookings = await res.json();

      const rawJobs = (bookings || []).filter(j =>
        (j.chosenWorkerRole || "").toLowerCase() === "plumber" &&
