  ...
  const name  = localStorage.getItem("name") || localStorage.getItem("userName") || "Carpenter";

  if (!phone || role !== "carpenter") {
    window.location.replace("login.html");
    throw new Error("Carpenter role required");
  }
  ...
  function renderRoleSidebar() {
    ...
    if (nav) {
      nav.innerHTML = `
        <a class="active" href="carpenter_requests.html"><i class="fa-solid fa-table-cells-large"></i>Dashboard</a>
    ...
    if (info) {
      info.innerHTML = `
        <div><i class="fa-solid fa-bolt"></i>Carpenter workspace</div>
  ...
  function setWorkerAvatar(imageValue, displayName) {
    ...
    document.querySelectorAll("[data-ns-avatar]").forEach(el => {
      el.innerHTML = imageSrc
        ? `<img src="${imageSrc}" alt="${displayName || "Carpenter"}">`
        : initial;
    });
  ...
  async function loadSidebarProfileAvatar() {
    try {
      ...
      if (profileResult.status === "fulfilled" && profileResult.value.ok) {
        ...
        console.log("Carpenter profile user:", user);
      }
      ...
      const profileImage = user.avatarBase64 ||
        ...
        "";
      const displayName = user.name || name || "Carpenter";
      ...
      console.error("Could not load carpenter sidebar avatar", e);
  ...
  function renderBooking(job){
    ...
    return `<div class="req-card ${st}">
      ...
      <div class="req-meta">
        <div class="req-meta-item">Phone: ${safe(cphone)}</div>
  ...
  async function fetchRequests(){
    ...
    try {
      const res = await fetch(`${API_BASE}/bookings/chosen/carpenter/${encodeURIComponent(phone)}`);
      const bookings = await res.json();

      const rawJobs = (bookings || []).filter(j =>
        (j.chosenWorkerRole || "").toLowerCase() === "carpenter" &&
